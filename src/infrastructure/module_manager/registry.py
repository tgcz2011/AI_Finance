...
def register_all_modules(registry: ModuleRegistry) -> None:
    modules = [
        ...
        ModuleMeta("trade_validator", "交易校验", "交易规则校验与拦截",
                    frozenset({"data_fetcher", "account", "config"}), True, True),
        ModuleMeta("trade_executor", "交易执行", "原子性交易执行与费用计算",
                    frozenset({"account", "loan", "wal", "config"}), True, True),
        ModuleMeta("risk", "风控引擎", "多规则风控检查与熔断",
                    frozenset({"account", "config"}), True, True),
        ModuleMeta("data_fetcher", "数据接入", "多市场数据拉取与容错",
                    frozenset({"config", "logging"}), True, False),
        ModuleMeta("ai_adapter", "AI适配", "AI决策请求与上下文压缩",
                    frozenset({"data_fetcher", "account", "config", "logging"}), True, False),
        ModuleMeta("loan", "借贷管理", "贷款发放计息与强平",
                    frozenset({"account", "trade_executor", "config"}), True, False),
        ModuleMeta("scheduler", "模拟调度", "实时/回测调度与多周期决策",
                    frozenset({"data_fetcher", "ai_adapter", "trade_validator",
                               "trade_executor", "risk", "loan", "config"}), True, False),
        ModuleMeta("contest", "竞赛管理", "多AI竞赛与多轮淘汰",
                    frozenset({"scheduler", "account", "config"}), True, False),
